...
public class MainMenu extends JFrame {
	...
	public void inittree() {
		lefttree = new JTree();
		lefttree.setBackground(UIManager.getColor("text"));
		lefttree.setFont(new Font("Arial", Font.PLAIN, 13));
	...
	public MainMenu() {
		...
		xuanxiangNewMenu.setFont(new Font("微软雅黑", Font.PLAIN, 14));
		...
		JMenuItem gaimimaMenuItem = new JMenuItem("更改密码");
        ...
        gongnengpanel.setBackground(SystemColor.inactiveCaptionBorder);
        ...
        pathtextField = new JTextField("Hello!Your HDFS path is hdfs://localhost:9000");
        pathtextField.setBounds(62, 502, 506, 33);
        ...
        lblNewLabel_1.setBounds(10, 502, 54, 33);
	...
	protected void makeNewDir(ActionEvent e) {
		...
		try {
			...
			if(nowpath.startsWith("hdfs")) {
				...
				JOptionPane.showMessageDialog(null, "请选择有效目录！", "提示", JOptionPane.ERROR_MESSAGE);
			}
		...
		try {
			...
			if(downfrom.startsWith("hdfs")) {
				...
				if(fs.isFile(path)) {
					...
					JOptionPane.showMessageDialog(null, "未选中可下载文件！", "下载当前文件", JOptionPane.ERROR_MESSAGE);
				}
	...
	protected void upload(ActionEvent e) {
		...
		String srcPath = "";
		String hdfsPath = pathtextField.getText() ;
		Path path = null;
		...
		try {
			...
			if(hdfsPath.startsWith("hdfs")) {
				...
				if(fs.isFile(path)) {
					JOptionPane.showMessageDialog(null, "请先选择可用文件夹！", "提示", JOptionPane.ERROR_MESSAGE);
				}else {
					JFileChooser jf = new JFileChooser();
